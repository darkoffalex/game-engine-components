...
nk_glfw3_device_create()
{
    ...
    if (len > 1) {
        ...
        glGetShaderInfoLog(dev->vert_shdr, len, nullptr, log);
    ...
    if (len > 1) {
        char *log = (char*)calloc((size_t)len, sizeof(char));
        glGetShaderInfoLog(dev->frag_shdr, len, nullptr, log);
        fprintf(stdout, "[GL]: failed to compile shader: %s", log);
        free(log);
    }
...

    {
        ...
        auto pos = (GLuint)dev->attrib_pos;
        auto uv = (GLuint)dev->attrib_uv;
        auto col = (GLuint)dev->attrib_col;
        ...
        glVertexArrayAttribFormat(dev->vao, pos, 2, GL_FLOAT, GL_FALSE, static_cast<GLuint>(vp));
        glVertexArrayAttribFormat(dev->vao, uv, 2, GL_FLOAT, GL_FALSE, static_cast<GLuint>(vt));
        glVertexArrayAttribFormat(dev->vao, col, 4, GL_UNSIGNED_BYTE, GL_TRUE, static_cast<GLuint>(vc));
    ...
    glNamedBufferStorage(dev->vbo, vb_size, nullptr, flags);
    glNamedBufferStorage(dev->ebo, eb_size, nullptr, flags);
...
{
    GLuint id;
    auto w = (GLsizei)width;
    auto h = (GLsizei)height;
    struct nk_glfw_device *dev = &glfw.ogl;
    ...
    else glClearTexImage(id, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
              ...
              int max_vertex_buffer, int max_element_buffer)
{
    ...
    nk_init_default(&glfw.ctx, nullptr);
    glfw.ctx.clip.copy = nk_glfw3_clipboard_copy;
    glfw.ctx.clip.paste = nk_glfw3_clipboard_paste;
    glfw.ctx.clip.userdata = nk_handle_ptr(nullptr);
